...
tell application "Keynote"
	...
	set extractBody to button returned of (display alert "Would you like to extract slide contents in addition to titles and presenter notes?" buttons {"Yes", "No"}) = "Yes"
...
set theFormattedNotes to "# " & " " & thePresentationName & return & return
repeat with a from 1 to length of theTitles
	...
	if extractBody = true then
		set slide to item a of theBodyText
		set txtList to paragraphs of slide
		repeat with txt in txtList
			set theFormattedNotes to theFormattedNotes & "* " & txt & return
		end repeat
